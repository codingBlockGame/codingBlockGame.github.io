...
function pushEntity() {
    if (player[0] !== null) drawData.push(player[0]);
    if (player[1] !== null) drawData.push(player[1]);
...
function drawGameScreen() {
    ...
    drawData = sort(drawData, (d1, d2) => {
        if (d1.className === 'Entity' && d2.className === 'Entity') {
            return (d2.deep.entity - d1.deep.entity);
        } else {
            return (d2.deep.block - d1.deep.block);
        };
    });

    for (let item of drawData) item.draw();
