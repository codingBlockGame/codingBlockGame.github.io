...
class Pos {
     ...
     */
    getDeep(size = {}) {
        function deepType(pos, type, size, sideSigned, unitDelta) {
            const fc = (type === 'block') ? ((n) => round(n)) : ((n) => (n));
            const vertex = Array.from({ length: 3 }, (_v, k) => unitDelta[k] * fc((pos[posTag[k]] + sideSigned[k] * ((size[posTag[k]] || blockSize) / 2 - smallNumber2)) / blockSize));
            const flat = Array.from({ length: 3 }, (_v, k) => (levelData.size * sum(unitDelta) - sum(vertex)) / sum(unitDelta) + vertex[k]);
            const deep = hypot(...Array.from({ length: 3 }, (_v, k) => abs(vertex[k], flat[k]))).toFixed(3) - 0;
            return deep;
        }

        function deepType2(pos, size, unitDelta) {
            const deep = sum(Array.from({ length: 3 }, (_v, k) => unitDelta[k] * (levelData.size * blockSize - (pos[posTag[k]] - ((size[posTag[k]] || playerSize) * 2 - (playerSize))))));
            return deep;
        }

        return {
            block: deepType(this, 'block', size, [1, -1, 1], [1, 1, 1]) * 99 + deepType(this, 'entity', size, [1, -1, 1], [1, 0.1, 1]),
            entity: deepType(this, 'entity', size, [-1, -1, -1], [1, 10, 1])
                // entity: deepType2(this, size, [1, 2, 1])
        };
    };
    /**
     * @param {...Number} posValue x, y, [...z]
...
class Block {
    ...
    constructor(type, pos) {
        ...
        this.deep = this.pos.getDeep({ x: blockSize, y: blockSize, z: blockSize });
    };
    draw() {
        const exp = new RegExp(`[^(${this.type})]|air`);
        ...
        for (let posId in drawTestPos) {
            if (/undefined/.test(blockPosId[drawTestPos[posId]]) || exp.test(blockPosId[drawTestPos[posId]].type)) drawBlock(this.id, this.pos, posTag[posId]);
        };
    };
};
...
class Entity {
    ...
    constructor(id, pos, tagData = null) {
        ...
        this.v = { x: 0, y: 0, z: 0 };
        this.deep = this.pos.getDeep(this.size);
        this.freezeTime = 0;
    ...
    setV(vx, vz) {
        this.v.x = (vx * (moveData.v1 + abs(vx) * abs(abs(vx) - abs(vz)) * moveData.v2)) * (deltaTime / 1000);
        this.v.z = (vz * (moveData.v1 + abs(vz) * abs(abs(vx) - abs(vz)) * moveData.v2)) * (deltaTime / 1000);
    };
    draw() {
        if (this.size.y > 0) {
